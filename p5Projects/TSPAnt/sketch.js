const numCities = 40;
const numAnts = 200;
const initPheromoneValue = 0.001;
const alpha = 5;
const beta = 1;
let cities = [];
let pheromoneMatrix;
...
function draw() {
  ...
  for (var N = 0; N < 20; N++) {
    ...
    updatePheromone(bestAnt, 0.0001);
  }
...
function updatePheromone(bestAnt, decay){
  ...
  for (var i = 0; i < bestAnt.path.length-1; i++) {
    pheromoneMatrix.updateSingle(bestAnt.path[i].index,bestAnt.path[i+1].index, bestAnt.distance * decay);
  }
