  ...
  this.addRect = function(rects){
    ...
    if(rect.owner == this.name){
      for (var i = 0; i<this.tempOwnedRects.length; i++) {
        this.tempOwnedRects[i].setOwner(this, true);
...
function ioPlayer(player){
  ...
  this.permOwnedRects = [];
  this.tempOwnedRects = [];
  for(var i = 0; i < player.tempOwnedRects.length; i++){
    var rect = {
      x: player.tempOwnedRects[i].pos.x,
      y: player.tempOwnedRects[i].pos.y,
      color: player.tempOwnedRects[i].color
    }
    this.tempOwnedRects.push(rect);
  }
  for(var i = 0; i < player.permOwnedRects.length; i++){
    var rect = {
      x: player.permOwnedRects[i].pos.x,
      y: player.permOwnedRects[i].pos.y,
      color: player.permOwnedRects[i].color
    }
    this.permOwnedRects.push(rect);
  }
